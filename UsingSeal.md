...
Run the server using `cargo run --bin key-server` either with the environment variable `CONFIG_PATH` pointing to a .yaml configuration file (see [crates/key-server/key-server-config.yaml] for an example) or by setting the required environment variables directly:
...
- `NETWORK` specifies the network to connect to, such as `testnet`, `mainnet`, or other supported environments. To use a custom full node, set `NETWORK` to `custom`. When using the `custom` option, you must also set the `NODE_URL` environment variable with the URL of your Full node endpoint.

...
It outputs the secret seed.


// TODO: TBD
For each allowed client, associate a unique `derivation_index` and use the `seal-cli` tool to print its that using `cargo run --bin seal-cli gen-seed`.
Key servers should be registered onchain to enable discoverability. To register a key server, call the `create_and_transfer_v1` function in the `seal::key_server` module. For example:

Create a configuration file with Permissioned mode and add your clients following the instructions from the [example](crates/key-server/key-server-config.yaml).

For every allowed client, 

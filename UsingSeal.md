...
The key server can run either in `Open` mode or `Permissioned`.
`Open` mode allows querying keys for any onchain package and uses a single IBE master key for all of them.
`Permissioned` mode restricts queries to manually approved packages, associated with a specific client. It also supports importing and exporting a key for the specific client if needed. More on both options in the following.

...
- **Open mode**: In open mode, the key server accepts decryption requests for any onchain package. It uses a single IBE master key to serve all access policies across packages. This mode is suitable for public or general-purpose deployments where package-level isolation is not required.
- **Permissioned mode**: In permissioned mode, the key server restricts access to a manually approved list of packages associated with a specific client or application. Each client is served using a dedicated IBE master key. This mode also supports importing or exporting the client-specific key if needed, for purposes such as disaster recovery or key server rotation.
...
### Open mode
In this mode Seal policies of any package can be queried.
This is an ideal option for testing Seal, or if the key server does not have any direct liabilty to users (*best-effort* service).

First, a master BLS keypair should be generated.
Use the `seal-cli` tool for that using `cargo run --bin seal-cli genkey`.
It outputs the master secret key and the public key.

Key servers should be registered onchain to enable discoverability.
To register a key server, call the `create_and_transfer_v1` function in the `seal::key_server` module. For example:
...
Run the server using `cargo run --bin key-server` with the environment variable 
`MASTER_KEY` set to the master secret key generated by the `seal-cli` tool,
and the environment variable `CONFIG_PATH` pointing to a .yaml configuration file. See [example](crates/key-server/key-server-config.yaml) for the format of the configuration file and use the `Open` mode. Set `key_server_object_id` to the id of the registered key server object.
...
Similarly, an health check endpoint can be used by calling
`curl http://0.0.0.0:2024/health`.

### Permissioned mode
In this mode only Seal policies of allowlisted packages can be queried.
This is the recommended option for B2B use cases.

First, a master seed should be generated.
Use the `seal-cli` tool for that using `cargo run --bin seal-cli gen-seed`.
It outputs the secret master seed.

Create a configuration file with `Permissioned` mode and no clients following the instructions from the [example](crates/key-server/key-server-config.yaml).
Run the server using `cargo run --bin key-server` with the environment variable 
`MASTER_SEED` set to the master secret seed generated by the `seal-cli` tool,
and the environment variable `CONFIG_PATH` pointing to a .yaml configuration file.
...
Each supported client requires registering an onchain key server object to enable discoverability.
For the first client, register a key server onchain with the first unassigned derived public key (with index 0).
To register a key server, call the `create_and_transfer_v1` function in the `seal::key_server` module as described above.
Then, add a client confguration to the config file with id of the registered key server object, set `client_master_key` to type `Derived`
with `derivation_index` `0`, and the packages used by the client.
You can run the server again `cargo run --bin key-server` to test your changes.

Adding more clients is done using a similar way:
Find the next unassigned derived public key from the server logs, register a key server with the next public key, and add a client configuration to the config file.

In the rare case in which exporting a key is required, you can use the `seal-cli` tool and run the command
`cargo run --bin seal-cli derive-key --seed $MASTER_SEED --index X` where `X` is the `derivation_index` of the
key you want to export. The command will output the BLS master key that can be imported by another server if needed.
To disable that key in your server, change `client_master_key` to `Exported` along with the `depreacted_derivation_index`.

When importing a BLS master key, add a new environment variable with the master key. Then, add a new client configuration 
with `client_master_key` of type `Imported`, the name of the new environment variable, the packages used by the client,
and the id of the registered key server object. Note that the registered object should be transfered to the owner of the
importing key server, and the new owner should update its url to point to the new key server.

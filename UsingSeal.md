...
Open mode allows quering keys for any onchain package and uses a single IBE master key for all of them.
...
It outputs the secret master key and the public key.

Key servers should be registered onchain to enable discoverability. To register a key server, call the `create_and_transfer_v1` function in the `seal::key_server` module. For example:
...
and the environment variable `CONFIG_PATH` pointing to a .yaml configuration file. See [example](crates/key-server/key-server-config.yaml) for the format of the configuration file and use the Open mode.

Example:
```shell
export MASTER_KEY="KYinoC5hVWeWqOUU9dw7PVHiROYFWB/nQZ55Kmytjig="
export export CONFIG_PATH="/Users/alice/seal/key-server-config.yaml"
...
Next, register the first key server onchain with the first unassigned derived public key (with index 0) to enable discoverability.
To register a key server, call the `create_and_transfer_v1` function in the `seal::key_server` module as described above.
Then, add a client confguration to the config file with id of the registered key server object, `client_master_key` of type `Derived`
...
Adding more clients is done using a similar process: 
Find the next unassigned derived public key from the server logs, register a key server with the next public key,
and add a client configuration to the config file.
...
To disable that key in your server, change the `client_master_key` to `Exported` along with the `depreacted_derivation_index`

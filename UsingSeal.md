...
- **Open mode**: In open mode, the key server accepts decryption requests for any onchain package. 
It uses a single master key to serve all access policies across packages. 
This mode is suitable for public or general-purpose deployments where package-level isolation is not required.
- **Permissioned mode**: In permissioned mode, the key server restricts access to a manually approved list of packages associated with a specific client or application.
Each client is served using a dedicated master key.
...
### Open mode
In `Open` mode, the key server allows decryption requests for Seal policies from any package.
This mode is ideal for testing or for deployments where the key server is operated as a best-effort 
service without direct user liability.

Before starting the key server, you must generate a BLS master key pair.
Use the `seal-cli` tool as `cargo run --bin seal-cli genkey`.
This command outputs both the master secret key and the public key.
...
### Permissioned mode
In `Permissioned` mode, the key server only allows decryption requests for Seal policies from explicitly allowlisted packages.
This is the recommended mode for B2B deployments where tighter access control and client-specific key separation are required.


Start by generating a master seed for the key server. 
Use the `seal-cli` tool as `cargo run --bin seal-cli gen-seed`.
This command outputs the secret master seed which should be stored securely.
...
Set the environment variable `MASTER_SEED` to the master secret seed generated by the `seal-cli` tool,
and the environment variable `CONFIG_PATH` pointing to a .yaml configuration file.
Run the server using `cargo run --bin key-server`.
...
- Use the first unassigned derived public key (with derivation index 0).
- Register a new key server on-chain by calling the `create_and_transfer_v1` function from the `seal::key_server` module (see Open Mode above for usage).
- In your key server configuration file:
...
Restart the key server to apply the changes, using `cargo run --bin key-server`.

To add more clients:
- Check the key server logs for the next unassigned derived public key.
- Register the new public key on-chain using `create_and_transfer_v1`.
- Add a new client configuration using the next `derivation_index` value and corresponding packages.

#### Export / Import Keys

In rare cases where you need to export a client key:
- Use the `seal-cli` tool as `cargo run --bin seal-cli derive-key --seed $MASTER_SEED --index X`. 
...
To import a BLS master key into a new key server:
- Set an environment variable with the imported key (e.g., `EXPORTED_CLIENT_KEY`).
- In the config file, add a new client entry with:
  - `client_master_key` set to type `Imported`.
  - The name of the environment variable containing the key.
  - The packages associated with the client.
  - The ID of the key server object registered on-chain for this client.

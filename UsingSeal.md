...
Before starting the key server, you must generate a BLS master key pair using CLI. This command outputs both the master secret key and the public key.
```
cargo run --bin seal-cli genkey
Master key: $MASTER_KEY
Public key: $MASATER_PUBKEY
```
...
```shell
sui client call --function create_and_transfer_v1 --module key_server --package 0x62c79dfeb0a2ca8c308a56bde530ccf3846535e1623949d45c90d23128afff52 --args $YOUR_SERVER_NAME_HERE https://your-url-here 0 $MASATER_PUBKEY --gas-budget 10000000
```

To start the key server in `Open` mode locally, run the server with the following command:
```
...
docker run -p 2024:2024 -e MASTER_KEY=$MASTER_KEY -e CONFIG_PATH="crates/key-server/key-server-config.yaml" seal-key-server
```
<!-- 
...
It should abort after printing a list of unassigned derived public keys (search for logs with the text `Unassigned derived public key`). For example:
```
...
- Use the `seal-cli` tool as `cargo run --bin seal-cli derive-key --seed $MASTER_SEED --index X`. Replace `X` with the `derivation_index` of the key you want to export.
